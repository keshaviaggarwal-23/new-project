...
//   },
      ...
      fetch(url)
        .then((response) => {
          return response.json().then((rowData) => {
            dispatch(actions.fetchGridDataSuccess(rowData));
          });
        })
        .catch((err) => {
  ...
  fetchGridDataSuccess(rowData) {
    return (dispatch) => {
      dispatch(actions.setGridData(rowData));
  ...
  changeTab(tabIndex) {
    return (dispatch, getState) => {
      ...
      if (fetchAgain) {
        dispatch(actions.fetchGridData(gridId, gridUrl));
      }
    };
  },
